...
class AVE_dataset(Dataset):
    def __init__(self, opt, mode="train"):
        ...
        if self.dataset_name == "vggsound":
            all_df = pd.read_csv(
                os.path.join(
                    self.root_path, "data/vggsound/vggsound-avel40k_labels.csv"
                )
            )
            ...
            with h5py.File(
                os.path.join(self.root_path, "data/AVE/labels.h5"), "r"
            ) as hf:
                self.labels = hf["avadataset"][:]

            if mode == "train":
                with h5py.File(
                    os.path.join(self.root_path, "data/AVE/train_order.h5"), "r"
                ) as hf:
                    order = hf["order"][:]
            elif mode == "test":
                with h5py.File(
                    os.path.join(self.root_path, "data/AVE/test_order.h5"), "r"
                ) as hf:
                    order = hf["order"][:]

            self.raw_gt = pd.read_csv(
                os.path.join(self.root_path, "data/AVE/Annotations.txt"), sep="&"
            )
            category_dict = dict()
            for category in self.all_categories:
                category_dict[category] = []
    ...
    def __getitem__(self, idx):
        if self.dataset_name == "vggsound":
            ...
            wave = np.load(
                os.path.join(
                    self.root_path, "data/vggsound/wave/{}.npy".format(file_name)
                ),
                allow_pickle=True,
            )
            ...
            # 需要查看一下wave的shape
            return {"GT": label, "image": total_img, "wave": wave}
            ...
            # import pdb
            # pdb.set_trace()
            filepath = "/data/AVE/wave"
            file_name += ".npy"
            filepath = os.path.join(self.root_path, filepath, file_name)
