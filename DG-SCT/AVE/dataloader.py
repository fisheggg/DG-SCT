...
class AVE_dataset(Dataset):
    def __init__(self, opt, mode="train"):
        self.opt = opt
        with h5py.File(
            os.path.join(self.opt.root_path, "data/AVE/mil_labels.h5"), "r"
        ) as hf:
            self.mil_labels = hf["avadataset"][:]

        with h5py.File(
            os.path.join(self.opt.root_path, "data/AVE/labels.h5"), "r"
        ) as hf:
            self.labels = hf["avadataset"][:]

        if mode == "train":
            with h5py.File(
                os.path.join(self.opt.root_path, "data/AVE/train_order.h5"), "r"
            ) as hf:
                order = hf["order"][:]
        elif mode == "test":
            with h5py.File(
                os.path.join(self.opt.root_path, "data/AVE/test_order.h5"), "r"
            ) as hf:
                order = hf["order"][:]
        ...
        self.raw_gt = pd.read_csv(
            os.path.join(self.opt.root_path, "data/AVE/Annotations.txt"), sep="&"
        )
    ...
    def __getitem__(self, idx):
        ...
        total_num_frames = len(
            glob.glob(
                os.path.join(self.opt.root_path, self.opt.video_folder)
                + "/"
                + file_name
                + "/*.jpg"
            )
        ...
        wave = np.load(
            os.path.join(self.opt.root_path, "data/AVE/wave/{}.npy".format(file_name)),
            allow_pickle=True,
        )
