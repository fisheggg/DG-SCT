...
class YBLoss2(torch.nn.Module):
    ...
    def forward(
        ...
        for i in range(x1.size(0)):
            ...
            if opt.aug_type == "vision":
                ...
                if i == 0:
                    ...
                    class_dic1 = torch.cat(
                        (class_dic1, x1_class[i].unsqueeze(0)), dim=0
                    )
                    class_dic2 = torch.cat(
                        (class_dic2, x2_class[i].unsqueeze(0)), dim=0
                    )
                ...
                gg_sample = torch.cat(
                    (
                        ...
                        (prob_x2[i + 1 * len(rand_idx)] * target[sample_idx[i]])
                        .max()
                        .unsqueeze(0),
                        (prob_x2[i + 2 * len(rand_idx)] * target[sample_idx[i]])
                        .max()
                        .unsqueeze(0),
                        (prob_x2[i + 3 * len(rand_idx)] * target[sample_idx[i]])
                        .max()
                        .unsqueeze(0),
                    )
                )
                gg_rand = torch.cat(
                    (
                        ...
                        (prob_x1[i + 1 * len(rand_idx)] * target[rand_idx[i]])
                        .max()
                        .unsqueeze(0),
                        (prob_x1[i + 2 * len(rand_idx)] * target[rand_idx[i]])
                        .max()
                        .unsqueeze(0),
                        (prob_x1[i + 3 * len(rand_idx)] * target[rand_idx[i]])
                        .max()
                        .unsqueeze(0),
                    )
                )
                ...
                loss.append(
                    F.binary_cross_entropy(
                        prob_x2[i + gg_sample.argmax() * len(rand_idx)],
                        target[sample_idx[i]],
                    )
                )
                loss.append(
                    F.binary_cross_entropy(
                        prob_x1[i + gg_rand.argmax() * len(rand_idx)],
                        target[rand_idx[i]],
                    )
                )
                loss.append(
                    F.binary_cross_entropy(
                        prob_joint[i + gg_joint.argmax() * len(rand_idx)], joint_label
                    )
                )
        ...
        if opt.aug_type == "vision":
            ...
            neg_bag = x2_prob_total_filiter[interval:-interval].sum(
                -1
            ) + x2_prob_total_filiter[-interval:].sum(-1)

            loss = (pos_bag / (neg_bag + pos_bag)).mean() + (
                1 - (neg_bag / (neg_bag + pos_bag)).mean()
            )
            ...
            neg = (
                torch.sum(torch.exp(sim / opt.tmp) * (1 - corr.detach()), dim=1) + 1e-10
            )
            ...
            corr = torch.mm(
                (target[rand_idx] + target[rand_idx]).clamp_(min=0, max=1),
                (target[rand_idx] + target[rand_idx])
                .clamp_(min=0, max=1)
                .permute(1, 0),
            )
            corr[corr != 0] = 1
...
class YBLoss(torch.nn.Module):
    ...
    def forward(self, all_prob, audio_idx, vis_idx, target, opt):
        v_prob_pos = torch.zeros(all_prob.shape[0] - len(audio_idx), 1, 25).to(
            all_prob.device
        )
        v_prob_neg = torch.zeros(all_prob.shape[0] - len(audio_idx), 1, 25).to(
            all_prob.device
        )
        v_prob_neg_counter = torch.zeros(all_prob.shape[0] - len(audio_idx), 1, 1).to(
            all_prob.device
        )

        a_prob_pos = torch.zeros(all_prob.shape[0] - len(audio_idx), 1, 25).to(
            all_prob.device
        )
        a_prob_neg = torch.zeros(all_prob.shape[0] - len(audio_idx), 1, 25).to(
            all_prob.device
        )
        a_prob_neg_counter = torch.zeros(all_prob.shape[0] - len(audio_idx), 1, 1).to(
            all_prob.device
        )
        ...
        for i in range(len(audio_idx)):
            if (target[audio_idx[i]] * target[vis_idx[i]]).sum() == 0:
                if opt.exp:
                    if opt.pos_pool == "max":
                        a_prob_pos[audio_idx[i]] = torch.exp(
                            all_prob[audio_idx[i], :, 0, :].max(0)[0]
                        )
                        v_prob_pos[vis_idx[i]] = torch.exp(
                            all_prob[vis_idx[i], :, 1, :].max(0)[0]
                        )
                    elif opt.pos_pool == "mean":
                        a_prob_pos[audio_idx[i]] = torch.exp(
                            all_prob[audio_idx[i], :, 0, :].mean(0)
                        )
                        v_prob_pos[vis_idx[i]] = torch.exp(
                            all_prob[vis_idx[i], :, 1, :].mean(0)
                        )

                    if opt.neg_pool == "max":
                        a_prob_neg[audio_idx[i]] += torch.exp(
                            all_prob[-len(audio_idx) + i, :, :, :].max(0)[0][1]
                        )
                        a_prob_neg_counter[audio_idx[i]] += 1
                        v_prob_neg[vis_idx[i]] += torch.exp(
                            all_prob[-len(audio_idx) + i, :, :, :].max(0)[0][0]
                        )
                        v_prob_neg_counter[vis_idx[i]] += 1
                    elif opt.neg_pool == "mean":
                        a_prob_neg[audio_idx[i]] += torch.exp(
                            all_prob[-len(audio_idx) + i, :, :, :].mean(0)[1]
                        )
                        a_prob_neg_counter[audio_idx[i]] += 1
                        v_prob_neg[vis_idx[i]] += torch.exp(
                            all_prob[-len(audio_idx) + i, :, :, :].mean(0)[0]
                        )
                        v_prob_neg_counter[vis_idx[i]] += 1
                else:
                    if opt.pos_pool == "max":
                        a_prob_pos[audio_idx[i]] = all_prob[audio_idx[i], :, 0, :].max(
                            0
                        )[0]
                        v_prob_pos[vis_idx[i]] = all_prob[vis_idx[i], :, 1, :].max(0)[0]
                    elif opt.pos_pool == "mean":
                        a_prob_pos[audio_idx[i]] = all_prob[audio_idx[i], :, 0, :].mean(
                            0
                        )
                        v_prob_pos[vis_idx[i]] = all_prob[vis_idx[i], :, 1, :].mean(0)

                    if opt.neg_pool == "max":
                        a_prob_neg[audio_idx[i]] += all_prob[
                            -len(audio_idx) + i, :, :, :
                        ].max(0)[0][1]
                        a_prob_neg_counter[audio_idx[i]] += 1
                        v_prob_neg[vis_idx[i]] += all_prob[
                            -len(audio_idx) + i, :, :, :
                        ].max(0)[0][0]
                        v_prob_neg_counter[vis_idx[i]] += 1
                    elif opt.neg_pool == "mean":
...
class MaskInfoNCELoss(torch.nn.Module):
    ...
    def forward(self, q, k, mask):
        ...
        # then output will be a b×n×p tensor.
        sim = torch.mm(q, k.T)
        tmp_zeros = torch.zeros((sim.shape[0] - mask.shape[0], sim.shape[1])).to(
            q.device
        )
        # if len(tmp_zeros) !=0:
        #     print('neg_work')
        ...
        neg_w = 1 - mask_pos
        pos = (
            torch.exp(torch.div(sim, self.τ)) * mask_pos
        )  # + torch.exp(torch.div(sim_vv,self.τ))*(mask-pos_w)
        pos = pos.sum(1)
        ...
        # performs matrix multiplication between query and queue tensors
        neg = torch.sum(
            torch.exp(torch.div(sim, self.τ)) * neg_w, dim=1
        )  # + torch.sum(torch.exp(torch.div(sim_vv,self.τ))*neg_w, dim=1)
