...
def visualization_temproal(results, path):
    ...
    category_colors = plt.get_cmap("hsv")(np.linspace(0, 1, len(category_names)))
    ...
    for legend_idx in range(25):
        ax.barh(
            ...
            left=0,
            height=0.5,
            label=category_names[legend_idx],
            color=category_colors[legend_idx],
        )
    for idx in range(10):
    ...
    ax.legend(ncol=10, bbox_to_anchor=(0, 1.15), loc="upper left", fontsize="small")
