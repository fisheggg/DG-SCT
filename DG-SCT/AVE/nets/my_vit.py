...
class my_vit(nn.Module):
    ...
    def __init__(self, name=""):
        ...
        my_conv = my_PatchEmbed(
            img_size=self.v.patch_embed.img_size[0],
            patch_size=self.v.patch_embed.patch_size[0],
            in_chans=3,
    ...
    def forward_patch(self, x, is_shape_info=False):
        ...
        ### standard vit -------->
        x = torch.cat((self.v.cls_token.expand(x.shape[0], -1, -1), x), dim=1)
        ### <---------
        ...
        if self.v.pos_embed.size(1) != x.size(1):
            x = self.v.pos_drop(
                x
                + F.interpolate(
                    self.v.pos_embed.permute(0, 2, 1), x.size(1), mode="linear"
                ).permute(0, 2, 1)
            )
        else:
