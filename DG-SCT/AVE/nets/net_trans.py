...
class TemporalAttention(nn.Module):
    ...
    def forward(self, visual_feature, audio_feature):
        ...
        video_query_output = (
            video_query_output + audio_gate * video_query_output * self.gamma
        )
        audio_query_output = (
            audio_query_output + video_gate * audio_query_output * self.gamma
        )
...
class CMBS(nn.Module):
    ...
    def forward(self, visual_feature, audio_feature):
        ...
        is_event_scores, event_scores = self.localize_module(
            (visual_feature + audio_feature) / 2
        )
...
class AudioVisualContrastive(nn.Module):
    ...
    def forward(self, video, audio, f_v_spatial_att_maps):
        ...
        visual_feat_before_grounding_posi = visual_feat_before_grounding_posi.view(
            bs, 10, C
        )  # [B, T, C]
        ...
        for i, audio_feat_aa_i in enumerate(audio_feat_aa):
            for j, (
                visual_feat_before_grounding_posi_j,
                visual_feat_posi_j,
            ) in enumerate(zip(visual_feat_before_grounding_posi, visual_feat_posi)):
                x2_va = torch.matmul(
                    visual_feat_posi_j, audio_feat_aa_i
                ).squeeze()  # [T, HxW]

                x2_p = F.softmax(x2_va, dim=-1).unsqueeze(-2)  # [T, 1, HxW]
                visual_feat_grd = torch.matmul(x2_p, visual_feat_posi_j)
                visual_feat_grd_after_grounding_posi = (
                    visual_feat_grd.squeeze()
                )  # [T, C]

                visual_gl_ij = torch.cat(
                    (
                        visual_feat_before_grounding_posi_j,
                        visual_feat_grd_after_grounding_posi,
                    ),
...
class AudioVisualAdapter(nn.Module):
    def __init__(self):
        ...
        self.audio_visual_rnn_layer = RNNEncoder(
            audio_dim=128, video_dim=512, d_model=self.d_model, num_layers=1
        )
...
class MMIL_Net(nn.Module):
    def __init__(self, opt):
        ...
        self.adapter_token_downsampler = nn.ModuleList(
            [
                nn.Linear(
                    down_out_dim[i] // (self.opt.Adapter_downsample * 2),
                    down_out_dim[i] // self.opt.Adapter_downsample,
                    bias=False,
                )
    ...
    def forward_swin(self, audio, vis, mixup_lambda, rand_train_idx=12, stage="eval"):
        ...
        is_event_scores, event_scores, av_score = self.CMBS(
