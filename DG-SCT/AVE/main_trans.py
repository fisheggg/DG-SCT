...
def train(args, model, train_loader, optimizer, criterion, criterion_event, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        if args.backbone_type == "audioset":
            mixup_lambda = torch.from_numpy(get_mix_lambda(0.5, len(wave) * 10)).to(
                "cuda"
            )
        else:
            mixup_lambda = None
        ...
        is_event_scores, event_scores, audio_visual_gate, av_score = model(
            [wave], image, rand_train_idx=rand_train_idx, stage="train"
        )
        ...
        # weights update
        if ((batch_idx + 1) % args.accum_itr == 0) or (
...
def main():
    ...
    if args.mode == "train":
        ...
        scheduler = optim.lr_scheduler.StepLR(
            optimizer, step_size=args.decay_epoch, gamma=args.decay
        )
        ...
        model.load_state_dict(
            torch.load(args.model_save_dir + "best_82.18" + ".pt"), strict=False
        )
