        ...
        if not os.path.exists(os.path.join(extract_frames_base_path, video_id)):
            ...
            for k, _ in enumerate(extract_frame):
                if k % sample_num == 15:
                    ...
                    cv2.imwrite(
                        os.path.join(
                            frame_save_path, video_name + "_" + str(count) + ".png"
                        ),
                        cv2.cvtColor(extract_frame[k], cv2.COLOR_RGB2BGR),
                    )
...
def extract_audio_wav(
    root_path="../avsbench_data/Multi-sources/ms3_data",
...
def extract_one_log_mel(wav_base_path, lm_save_base_path, df_one_video):
    ...
    print(
        "video_id: {}, log_mel_tensor.shape: {}".format(
            video_name, log_mel_tensor.shape
        )
    )
    return wrong_list
    ...
    for i in tqdm(range(lenth_df)):
        try:
            wrong_item = extract_one_log_mel(
                wav_path, lm_save_base_path, df_anno.iloc[i]
            )
