...
def video_frame_sample(frame_interval, video_length, sample_num):
    ...
    for l in range(video_length):
        for i in range(sample_num):
            num.append(
                int(l * frame_interval + (i * 1.0 / sample_num) * frame_interval)
            )
    return num


def extract_frame_for_each_video(root_path, trimed_video_base_path, df_one_video):
    video_name, start_time, category, split = (
        df_one_video[0],
        df_one_video[1],
        df_one_video[2],
        df_one_video[3],
    )
    trimed_video_path = os.path.join(
        trimed_video_base_path, split, category, video_name + ".mp4"
    )
    ...
    frame_save_path = os.path.join(
        extract_frames_base_path, split, category, video_name
    )
    ...
    for k, _ in enumerate(extract_frame):
        if k % sample_num == 15:
            count += 1
            cv2.imwrite(
                os.path.join(frame_save_path, video_name + "_" + str(count) + ".png"),
                cv2.cvtColor(extract_frame[k], cv2.COLOR_RGB2BGR),
            )


def extract_frames(
    root_path="../avsbench_data/Single-source/s4_data",
    ...
    for item in AVSBenchDataset:
        ...
        for i in tqdm(range(len(df_one_class))):
            try:
                extract_frame_for_each_video(
                    root_path, trimed_video_path, df_one_class.iloc[i]
                )
                count += 1
...
def split_audio(trimed_video_base_path, wav_save_base_path, df_one_video):
    ...
    wav_save_path = os.path.join(
        wav_save_base_path, split, category, video_name + ".wav"
    )
...
def extract_audio_log_mel(
    ...
    for item in AVSBenchDataset:
        ...
        for i in tqdm(range(len(df_one_class))):
            try:
                wrong_item = extract_one_log_mel(
                    wav_path, lm_save_base_path, df_one_class.iloc[i]
                )
