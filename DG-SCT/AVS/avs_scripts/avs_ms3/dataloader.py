...
class MS3Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        audio_lm_path = os.path.join(
            cfg.DATA.DIR_AUDIO_LOG_MEL, self.split, video_name + ".pkl"
        )
        ...
        for audio_sec in range(5):
            fbank, mix_lambda = self._wav2fbank(
                os.path.join(cfg.DATA.DIR_AUDIO_WAV, self.split, video_name + ".wav"),
                idx=audio_sec,
            )
        ...
        wave = np.load(
            os.path.join(
                self.opt.root_path,
                "data/AVSBench_data/Multi-sources/ms3_data/wave",
                self.split,
                "AVS.npy",
            ),
            allow_pickle=True,
