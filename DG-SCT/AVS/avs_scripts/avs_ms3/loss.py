...
def A_MaskedV_SimmLoss(
    ...
    for stage in count_stages:
        ...
        if threshold:
            downsample_pred_masks = (
                downsample_pred_masks > 0.5
            ).float()  # [bs*5, 1, H, W]
        ...
        if euclidean_flag:
            ...
            loss = F.kl_div(
                masked_v_fea.softmax(dim=-1).log(),
                a_fea.softmax(dim=-1),
                reduction="sum",
            )
        total_loss += loss
...
def IouSemanticAwareLoss(
    ...
    if sa_loss_flag:
        if closer_flag:  # Loss_AVM_VV reported in the paper
            ...
            masked_av_loss = A_MaskedV_SimmLoss(
                pred_masks,
                a_fea_list,
                v_map_list,
                count_stages,
