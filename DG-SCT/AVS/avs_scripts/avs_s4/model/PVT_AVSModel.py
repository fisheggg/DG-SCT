...
class CrossModalRelationAttModule(nn.Module):
    def __init__(self, input_dim, d_model, feedforward_dim):
        ...
        self.decoder_layer = DecoderLayer(
            d_model=d_model, nhead=4, dim_feedforward=feedforward_dim
        )
...
class VisualAdapter(nn.Module):
    ...
    def __init__(
        ...
        self.temporal_gated = nn.Sequential(nn.Linear(linear_out, 1), nn.Sigmoid())
        ...
        if adapter_kind == "bottleneck" and self.is_multimodal:
            ...
            self.down_sampler = nn.Conv2d(
                ...
                1,
                groups=self.opt.num_conv_group,
                bias=False,
            )
    ...
    def forward(self, x, vis_token=None):
        ...
        vis_token = F.interpolate(
            rearrange(vis_token, "BF w h c -> BF c w h"),
            mode="bicubic",
            size=[int(math.sqrt(self.conv_dim_out)), int(math.sqrt(self.conv_dim_out))],
        )
        ...
        if self.adapter_kind == "bottleneck" and self.is_multimodal:
            ...
            att_tk2x = torch.bmm(
                x.squeeze(-1).permute(0, 2, 1), rep_token.permute(0, 2, 1)
            )
            ...
            video_query_1 = F.relu(
                self.fc_affine_video_1(x.squeeze(-1).permute(0, 2, 1))
            )  # [*, grid ** 2, width]
            audio_video_query_raw = (audio_query_1 * video_query_1).mean(
                -2
            )  #  [*, width]
            audio_video_query = F.relu(self.fc_affine_bottleneck(audio_video_query_raw))
            channel_att_maps = (
                self.fc_affine_v_c_att(audio_video_query)
                .sigmoid()
                .reshape(x.size(0), 1, -1)
            )
            c_att_visual_feat = x.squeeze(-1).permute(0, 2, 1) * (
                channel_att_maps + 1
            )  # [B*10, 36, 768]
            ...
            spatial_att_maps = self.fc_softmax(
                self.fc_tanh(spatial_att_maps_tmp).transpose(2, 1)
            )
            ...
            # x = x.squeeze(-1).permute(0, 2, 1)
            # bs = x.size(0) // 5
            # x = x.view(bs, 5, x.size(-2), x.size(-1)) # [B, 10, HxW, C]
            # channel_att_maps_tmp = channel_att_maps.view(bs, 5, channel_att_maps.size(-2), channel_att_maps.size(-1))
            # spatial_att_maps_sigmoid_tmp = spatial_att_maps_sigmoid.view(bs, 5, spatial_att_maps_sigmoid.size(-2), spatial_att_maps_sigmoid.size(-1))
            ...
            if self.opt.is_before_layernorm:
                x = (
                    self.ln_before(x.squeeze(-1).permute(0, 2, 1))
                    .permute(0, 2, 1)
                    .unsqueeze(-1)
                )
        ...
        if self.opt.is_post_layernorm:
            output = (
                self.ln_post(output.squeeze(-1).permute(0, 2, 1))
                .permute(0, 2, 1)
                .unsqueeze(-1)
            )
...
class TemporalAttention(nn.Module):
    def __init__(self):
        ...
        self.v_fc = nn.ModuleList(
            [nn.Linear(self.video_input_dim, self.video_fc_dim) for i in range(4)]
        )
        ...
        self.video_encoder = nn.ModuleList(
            [
                InternalTemporalRelationModule(
                    input_dim=512, d_model=self.d_model, feedforward_dim=1024
                )
                for i in range(4)
            ]
        )
        self.video_decoder = nn.ModuleList(
            [
                CrossModalRelationAttModule(
                    input_dim=512, d_model=self.d_model, feedforward_dim=1024
                )
                for i in range(4)
            ]
        ...
        self.audio_encoder = nn.ModuleList(
            [
                InternalTemporalRelationModule(
                    input_dim=self.d_model, d_model=self.d_model, feedforward_dim=1024
                )
                for i in range(4)
            ]
        )
        ...
        self.audio_visual_rnn_layer = nn.ModuleList(
            [
                RNNEncoder(
        ...
        self.audio_gated = nn.ModuleList(
            [nn.Sequential(nn.Linear(self.d_model, 1), nn.Sigmoid()) for i in range(4)]
        )
        self.video_gated = nn.ModuleList(
            [nn.Sequential(nn.Linear(self.d_model, 1), nn.Sigmoid()) for i in range(4)]
        )
    ...
    def forward(self, visual_feature_list, audio_feature):
        ...
        x1_, x2_, x3_, x4_ = [
            self.v_fc[i](x) for i, x in enumerate([x1_, x2_, x3_, x4_])
        ]
        x1_, x2_, x3_, x4_ = [self.dropout(self.relu(x)) for x in [x1_, x2_, x3_, x4_]]

        visual_rnn_input = [x1_, x2_, x3_, x4_]

        audio_rnn_output1, visual_rnn_output1 = self.audio_visual_rnn_layer[0](
            audio_rnn_input, visual_rnn_input[0]
        )
        audio_rnn_output2, visual_rnn_output2 = self.audio_visual_rnn_layer[1](
            audio_rnn_input, visual_rnn_input[1]
        )
        audio_rnn_output3, visual_rnn_output3 = self.audio_visual_rnn_layer[2](
            audio_rnn_input, visual_rnn_input[2]
        )
        audio_rnn_output4, visual_rnn_output4 = self.audio_visual_rnn_layer[3](
            audio_rnn_input, visual_rnn_input[3]
        )

        audio_encoder_input1 = audio_rnn_output1.transpose(
            1, 0
        ).contiguous()  # [5, B, 256]
        audio_encoder_input2 = audio_rnn_output2.transpose(
            1, 0
        ).contiguous()  # [5, B, 256]
        audio_encoder_input3 = audio_rnn_output3.transpose(
            1, 0
        ).contiguous()  # [5, B, 256]
        audio_encoder_input4 = audio_rnn_output4.transpose(
            1, 0
        ).contiguous()  # [5, B, 256]

        visual_encoder_input1 = visual_rnn_output1.transpose(
            1, 0
        ).contiguous()  # [5, B, 512]
        visual_encoder_input2 = visual_rnn_output2.transpose(
            1, 0
        ).contiguous()  # [5, B, 512]
        visual_encoder_input3 = visual_rnn_output3.transpose(
            1, 0
        ).contiguous()  # [5, B, 512]
        visual_encoder_input4 = visual_rnn_output4.transpose(
            1, 0
        ).contiguous()  # [5, B, 512]
        ...
        audio_query_output1 = self.audio_decoder[0](
            audio_encoder_input1, video_key_value_feature1
        )
        audio_query_output2 = self.audio_decoder[1](
            audio_encoder_input2, video_key_value_feature2
        )
        audio_query_output3 = self.audio_decoder[2](
            audio_encoder_input3, video_key_value_feature3
        )
        audio_query_output4 = self.audio_decoder[3](
            audio_encoder_input4, video_key_value_feature4
        )
        ...
        video_query_output1 = self.video_decoder[0](
            visual_encoder_input1, audio_key_value_feature1
        )
        video_query_output2 = self.video_decoder[1](
            visual_encoder_input2, audio_key_value_feature2
        )
        video_query_output3 = self.video_decoder[2](
            visual_encoder_input3, audio_key_value_feature3
        )
        video_query_output4 = self.video_decoder[3](
            visual_encoder_input4, audio_key_value_feature4
        )
...
class Pred_endecoder(nn.Module):
    ...
    def __init__(
        ...
        if opt.backbone_type == "esc-50":
            ...
            esc_config.checkpoint_path = os.path.join(
                opt.root_path, "DG-SCT/checkpoints/ESC-50/"
            )
            ...
            esc_config.checkpoint_path = os.path.join(
                opt.root_path, "DG-SCT/checkpoints/AudioSet/"
            )
            ...
            esc_config.checkpoint_path = os.path.join(
                opt.root_path, "DG-SCT/checkpoints/SCV2/"
            )
        ...
        checkpoint_path = os.path.join(
            esc_config.checkpoint_path, esc_config.checkpoint
        )
        ...
        ## ------------> for swin and htsat
        for idx_layer, (my_blk, my_blk_a) in enumerate(
            zip(self.swin.layers, self.htsat.layers)
        ):
        ...
        self.audio_adapter_blocks_p1 = nn.ModuleList(
            [
                VisualAdapter(
                    input_dim=hidden_list_a[i],
                    output_dim=hidden_list_a[i],
                    adapter_kind="bottleneck",
                    dim_list=hidden_list_a,
                    layer_idx=i,
                    reduction_factor=self.opt.Adapter_downsample,
                    opt=opt,
                    use_bn=self.opt.is_bn,
                    use_gate=self.opt.is_gate,
                    conv_dim_in=conv_dim[i],
                    conv_dim_out=conv_dim_a[i],
                    linear_in=hidden_list[i],
                    linear_out=hidden_list_a[i],
                )
        ...
        self.vis_adapter_blocks_p1 = nn.ModuleList(
            [
                VisualAdapter(
                    ...
                    use_bn=self.opt.is_bn,
                    use_gate=True,
                    conv_dim_in=conv_dim_a[i],
                    conv_dim_out=conv_dim[i],
                    linear_in=hidden_list_a[i],
                    linear_out=hidden_list[i],
                )
    ...
    def forward(self, x, audio_feature=None, mixup_lambda=None):
        ...
        if audio.shape[2] > self.htsat.freq_ratio * self.htsat.spec_size:
            audio = self.htsat.crop_wav(
                audio, crop_size=self.htsat.freq_ratio * self.htsat.spec_size
            )
        ...
        for _, (my_blk, htsat_blk) in enumerate(
            ...
            for blk, blk_a in zip(my_blk.blocks, aud_blocks):
                if blk_a is not None:
                    f_a_res, f_a_spatial_att_maps = self.audio_adapter_blocks_p1[
                        idx_layer
                    ](
                        f_a.permute(0, 2, 1).unsqueeze(-1),
                        f_v.permute(0, 2, 1).unsqueeze(-1),
                    )
                    f_v_res, f_v_spatial_att_maps = self.vis_adapter_blocks_p1[
                        idx_layer
                    ](
                        f_v.permute(0, 2, 1).unsqueeze(-1),
                        f_a.permute(0, 2, 1).unsqueeze(-1),
                    ...
                    f_a_res, f_a_spatial_att_maps = self.audio_adapter_blocks_p2[
                        idx_layer
                    ](
                        f_a.permute(0, 2, 1).unsqueeze(-1),
                        f_v.permute(0, 2, 1).unsqueeze(-1),
                    )
                    f_v_res, f_v_spatial_att_maps = self.vis_adapter_blocks_p2[
                        idx_layer
                    ](
                        f_v.permute(0, 2, 1).unsqueeze(-1),
                        f_a.permute(0, 2, 1).unsqueeze(-1),
        ...
        audio_feature = rearrange(
            torch.bmm(f_a_spatial_att_maps, f_a).squeeze(dim=1), "(b t) d -> b t d", t=5
        )
        ...
        x1 = F.interpolate(
            rearrange(x1, "BF w h c -> BF c w h"), mode="bicubic", size=[56, 56]
        )
        x2 = F.interpolate(
            rearrange(x2, "BF w h c -> BF c w h"), mode="bicubic", size=[28, 28]
        )
        x3 = F.interpolate(
            rearrange(x3, "BF w h c -> BF c w h"), mode="bicubic", size=[14, 14]
        )
        x4 = F.interpolate(
            rearrange(x4, "BF w h c -> BF c w h"), mode="bicubic", size=[7, 7]
        )
        ...
        ############## Temporal Attention #################
        feature_map_list, audio_feature = self.temporal_attn(
            [x1, x2, x3, x4], audio_feature
        )
