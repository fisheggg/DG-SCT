...
if __name__ == "__main__":
    ...
    print(
        "####### Total audio params in M: %0.1f M  #######"
        % (total_params_audio / 1000000)
    )
    ...
    for epoch in range(args.max_epoches):
        for n_iter, batch_data in enumerate(train_dataloader):
            ...
            if args.backbone_type == "audioset":
                mixup_lambda = torch.from_numpy(get_mix_lambda(0.5, len(wave) * 5)).to(
                    "cuda"
                )
            else:
                mixup_lambda = None
            ...
            output, visual_map_list, a_fea_list = model(
                imgs, wave, mixup_lambda=mixup_lambda
            )  # [bs*5, 1, 224, 224]

            # print("output:")
            # print(output.shape)
            # print("visual_map_list:")
            # print(len(visual_map_list))
            # print("a_fea_list")
            # print(len(a_fea_list))
            ...
            if (global_step - 1) % 50 == 0:
                train_log = (
                    "Iter:%5d/%5d, Total_Loss:%.4f, iou_loss:%.4f, sa_loss:%.4f, lambda_1:%.4f, lr: %.6f"
                    % (
                        global_step - 1,
                        max_step,
            ...
            for n_iter, batch_data in enumerate(val_dataloader):
                ...
                if args.backbone_type == "audioset":
                    mixup_lambda = torch.from_numpy(
                        get_mix_lambda(0.5, len(wave) * 10)
                    ).to("cuda")
                else:
                    mixup_lambda = None
