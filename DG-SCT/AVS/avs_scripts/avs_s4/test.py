...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for n_iter, batch_data in enumerate(test_dataloader):
            imgs, audio_spec, audio, mask, wave, category_list, video_name_list = (
            ...
            )  # [5, 1, 224, 224] = [bs=1 * T=5, 1, 224, 224]
            if args.save_pred_mask:
                mask_save_path = os.path.join(log_dir, "pred_masks")
                save_mask(
                    output.squeeze(1), mask_save_path, category_list, video_name_list
                )
