...
class S4Dataset(Dataset):
    ...
    def __init__(self, split="train", args=None):
        ...
        print(
            "{}/{} videos are used for {}".format(
                len(self.df_split), len(df_all), self.split
            )
        )
    ...
    def _wav2fbank(self, filename, filename2=None, idx=None):
        ...
        sample_indx = np.linspace(
            0, waveform.shape[1] - sr * (self.opt.audio_length + 0.1), num=5, dtype=int
        )
        waveform = waveform[
            :, sample_indx[idx] : sample_indx[idx] + int(sr * self.opt.audio_length)
        ]
    ...
    def __getitem__(self, index):
        ...
        audio_lm_path = os.path.join(
            cfg.DATA.DIR_AUDIO_LOG_MEL, self.split, category, video_name + ".pkl"
        )
        mask_base_path = os.path.join(
            cfg.DATA.DIR_MASK, self.split, category, video_name
        )
        ...
        for audio_sec in range(5):
            fbank, mix_lambda = self._wav2fbank(
                os.path.join(
                    cfg.DATA.DIR_AUDIO_WAV, self.split, category, video_name + ".wav"
                ),
                idx=audio_sec,
            )
        ...
        wave = np.load(
            os.path.join(
                self.opt.root_path,
                "data/AVSBench_data/Single-source/s4_data/wave",
                self.split,
                category,
                "AVS.npy",
            ),
        ...
        if self.split == "train":
            return imgs_tensor, total_audio, audio_log_mel, masks_tensor, wave
        else:
            return (
                imgs_tensor,
                total_audio,
...
if __name__ == "__main__":
    ...
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=2, shuffle=False, num_workers=8, pin_memory=True
    )
