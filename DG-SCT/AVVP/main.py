...
def train(args, model, train_loader, optimizer, criterion, criterion_ce, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        if args.backbone_type == "audioset":
            mixup_lambda = torch.from_numpy(get_mix_lambda(0.5, len(audio) * 10)).to(
                "cuda"
            )
        else:
            mixup_lambda = None
        ...
        (
            ...
            a_frame_prob,
            v_frame_prob,
        ) = model(audio, video, video_st, mixup_lambda=mixup_lambda)
        ...
        if batch_idx % args.log_interval == 0:
            print(
                "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tLoss av: {:.6f}\tLoss a: {:.6f}\tLoss v: {:.6f}\tLoss cls_aud: {:.6f}\tLoss cls_vis: {:.6f}".format(
                    epoch,
                    batch_idx * len(audio),
...
def eval(args, model, val_loader, set):
    ...
    df_a = pd.read_csv(
        os.path.join(grandparent_dir, "data/AVVP/AVVP_eval_audio.csv"),
        header=0,
        sep="\t",
    )
    df_v = pd.read_csv(
        os.path.join(grandparent_dir, "data/AVVP/AVVP_eval_visual.csv"),
        header=0,
        sep="\t",
    )
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(val_loader):
            ...
            if args.backbone_type == "audioset":
                mixup_lambda = torch.from_numpy(
                    get_mix_lambda(0.5, len(audio) * 10)
                ).to("cuda")
            else:
                mixup_lambda = None

            _, _, output, a_prob, v_prob, a_frame_prob, v_frame_prob = model(
                audio, video, video_st, mixup_lambda=mixup_lambda
            )
            ...
            Pa = (Pa >= 0.5).astype(np.int_) * np.repeat(
                o, repeats=10, axis=0
            )  # [10, 25]
            Pv = (Pv >= 0.5).astype(np.int_) * np.repeat(
                o, repeats=10, axis=0
            )  # [10, 25]
    ...
    print(
        "Audio Event Detection Segment-level F1: {:.1f}".format(
            100 * np.mean(np.array(F_seg_a))
        )
    )
    print(
        "Visual Event Detection Segment-level F1: {:.1f}".format(
            100 * np.mean(np.array(F_seg_v))
        )
    )
    print(
        "Audio-Visual Event Detection Segment-level F1: {:.1f}".format(
    ...
    print(
        "Audio Event Detection Event-level F1: {:.1f}".format(
            100 * np.mean(np.array(F_event_a))
        )
    )
    print(
        "Visual Event Detection Event-level F1: {:.1f}".format(
            100 * np.mean(np.array(F_event_v))
        )
    )
    print(
        "Audio-Visual Event Detection Event-level F1: {:.1f}".format(
            100 * np.mean(np.array(F_event_av))
        )
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PyTorch Implementation of Multimodal Grouping Network for AVVP"
    )
    parser.add_argument(
        "--audio_dir",
        type=str,
        default="data/AVVP/LLP_dataset/feats/vggish/",
        help="audio dir",
    )
    parser.add_argument(
        "--video_dir",
        type=str,
        default="data/AVVP/LLP_dataset/feats/res152/",
        help="video dir",
    )
    parser.add_argument(
        "--st_dir",
        type=str,
        default="data/AVVP/LLP_dataset/feats/r2plus1d_18/",
        help="video dir",
    )
    parser.add_argument(
        "--label_train",
        type=str,
        default="data/AVVP/AVVP_train.csv",
        help="weak train csv file",
    )
    parser.add_argument(
        "--label_val",
        type=str,
        default="data/AVVP/AVVP_val_pd.csv",
        help="weak val csv file",
    )
    parser.add_argument(
        "--label_test",
        type=str,
        default="data/AVVP/AVVP_test_pd.csv",
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="MGN_Net", help="save model name"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="soft",
        help="type of unimodal grouping assignment",
    )
    parser.add_argument(
        "--crossmodal_assign",
        type=str,
        default="soft",
        help="type of crossmodal grouping assignment",
    )
    ...
    parser.add_argument(
        "--audio_folder",
        type=str,
        default="data/AVVP/LLP_dataset/audio",
        help="raw audio path",
    )
    parser.add_argument(
        "--video_folder",
        type=str,
        default="data/AVVP/LLP_dataset/frame",
        help="video frame path",
    )
    ...
    print(
        "####### Trainable params: %0.4f  #######" % (train_params * 100 / total_params)
    )
    print(
        "####### Additional params: %0.4f  ######"
        % (additional_params * 100 / (total_params - additional_params))
    )
    ...
    if args.mode == "train":
        ...
        for epoch in range(0, args.epochs):
            ...
            scheduler.step()
            F, F_event = eval(
                args, model, val_loader, os.path.join(args.root_path, args.label_val)
            )
