...
class TemporalAttention(nn.Module):
    def __init__(self):
        ...
        self.audio_encoder = InternalTemporalRelationModule(
            input_dim=self.audio_input_dim, d_model=self.d_model, feedforward_dim=1024
        )
...
class MGN_Net(nn.Module):
    ...
    def forward(self, audio, visual, visual_st, mixup_lambda=None):
        ...
        x2, attn_visual_dict, _ = self.visual_cug(
            x2_0, self.visual_token, return_attn=True
        )

        # audio uni-modal grouping
        x1, attn_audio_dict, _ = self.audio_cug(
            x1_0, self.audio_token, x2_0, return_attn=True
        )
