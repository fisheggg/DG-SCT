...
class AVQA_dataset(Dataset):
    def __init__(
        ...
        samples = json.load(
            open(os.path.join(root_path, "data/AVQA/json/avqa-train.json"), "r")
        )
    ...
    def _wav2fbank(self, filename, filename2=None, idx=None):
        ...
        if waveform.shape[1] > 16000 * (1.95 + 0.1):
            sample_indx = np.linspace(
                0, waveform.shape[1] - 16000 * (1.95 + 0.1), num=10, dtype=int
            )
            waveform = waveform[
                :, sample_indx[idx] : sample_indx[idx] + int(16000 * 1.95)
            ]
    ...
    def __getitem__(self, idx):
        ...
        total_num_frames = len(
            glob.glob(
                os.path.join(self.root_path, self.video_res14x14_dir, name, "*.jpg")
            )
        )
        ...
        for tmp_idx in sample_indx:
            tmp_img = (
                torchvision.io.read_image(
                    os.path.join(
                        self.root_path,
                        self.video_res14x14_dir,
                        name,
                        str("{:08d}".format(tmp_idx)) + ".jpg",
                    )
                )
                / 255
            )
        ...
        for i in range(total_img.shape[0]):
            ...
            total_num_frames = len(
                glob.glob(
                    os.path.join(
                        self.root_path, self.video_res14x14_dir, neg_video_name, "*.jpg"
                    )
                )
            )
            sample_indx = np.linspace(1, total_num_frames, num=10, dtype=int)
            ...
            tmp_img = (
                torchvision.io.read_image(
                    os.path.join(
                        ...
                        self.video_res14x14_dir,
                        neg_video_name,
                        str("{:08d}".format(sample_indx[neg_frame_flag])) + ".jpg",
                    )
                )
                / 255
            )
        ...
        wave = np.load(
            os.path.join(self.root_path, "data/AVQA/audio_wave/{}.npy".format(name))
        )
