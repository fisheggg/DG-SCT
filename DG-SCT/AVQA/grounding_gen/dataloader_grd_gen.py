...
class AVQA_dataset(Dataset):
    def __init__(self, label_data, audio_dir, video_dir, transform=None):
        samples = json.load(
            open("/root/autodl-tmp/data/AVQA/json/avqa-train_real.json", "r")
        )
    ...
    def __getitem__(self, idx):
        ...
        total_num_frames = len(
            glob.glob(os.path.join(self.video_dir, pos_video_name, "*.jpg"))
        )
        ...
        ### ---> video frame process
        total_num_frames = len(
            glob.glob(os.path.join(self.video_dir, neg_video_name, "*.jpg"))
        )
        sample_indx = np.linspace(1, total_num_frames, num=10, dtype=int)
        total_img = []
        for tmp_idx in sample_indx:
        ...
        pos_wave = np.load(
            "/root/autodl-tmp/data/AVQA/audio_wave/{}.npy".format(pos_video_name)
        )
