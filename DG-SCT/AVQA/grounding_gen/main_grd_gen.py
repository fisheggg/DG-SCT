...
def train(args, model, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        writer.add_scalar(
            "train/grd_gen_loss", loss.item(), epoch * len(train_loader) + batch_idx
        )
...
def eval(args, model, val_loader, epoch):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(val_loader):
            ...
            if args.backbone_type == "audioset":
                mixup_lambda = torch.from_numpy(get_mix_lambda(0.5, len(audio))).to(
                    "cuda"
                )
            else:
                mixup_lambda = None
            ...
            total += preds.size(0)
            correct += (
                (predicted == target.view(target.size(0) * target.size(1))).sum().item()
            )
...
def main():
    ...
    parser.add_argument(
        "--audio_dir",
        type=str,
        default="/root/autodl-tmp/data/AVQA/audio",
        help="audio dir",
    )
    parser.add_argument(
        "--video_dir",
        type=str,
        default="/root/autodl-tmp/data/AVQA/frames",
        help="video dir",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--label_train",
        type=str,
        default="/root/autodl-tmp/data/AVQA/json/avqa-train_real.json",
        help="train csv file",
    )
    parser.add_argument(
        "--label_val",
        type=str,
        default="/root/autodl-tmp/data/AVQA/json/avqa-val_real.json",
        help="val csv file",
    )
    parser.add_argument(
        "--label_test",
        type=str,
        default="/root/autodl-tmp/data/AVQA/json/avqa-test_real.json",
        help="test csv file",
    )
    parser.add_argument(
        "--batch-size",
    ...
    parser.add_argument(
        "--model_save_dir",
        type=str,
        default="grounding_gen/models_grounding_gen/",
        help="model save dir",
    )
    parser.add_argument(
    ...
    parser.add_argument("--wandb", type=int, default=0, help="weight and bias setup")
    ...
    parser.add_argument(
        "--is_audio_adapter_p1", type=int, default=0, help="TF audio adapter"
    )
    parser.add_argument(
        "--is_audio_adapter_p2", type=int, default=0, help="TF audio adapter"
    )
    parser.add_argument(
    ...
    parser.add_argument("--tmp_av", type=float, default=0.1, help="tmp for nce")
    parser.add_argument("--tmp_tv", type=float, default=0.1, help="tmp for nce")

    parser.add_argument("--coff_av", type=float, default=0.5, help="tmp for nce")
    parser.add_argument("--coff_tv", type=float, default=0.5, help="tmp for nce")
    parser.add_argument(
    ...
    if args.mode == "train":
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if F >= best_F:
                ...
                torch.save(
                    model.state_dict(),
                    args.model_save_dir + args.checkpoint + "_best.pt",
                )
            torch.save(
                model.state_dict(),
                args.model_save_dir + args.checkpoint + str(epoch) + ".pt",
            )
        ...
        model.load_state_dict(
            torch.load(args.model_save_dir + args.checkpoint + "_best.pt")
        )
        eval(args, model, test_loader, epoch=epoch)
    else:
        test_dataset = AVQA_dataset(
            label_data=args.label_test,
            audio_dir=args.audio_dir,
            video_dir=args.video_dir,
        )
