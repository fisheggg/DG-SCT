...
def generate_category_list(root_path, dataset_name):
    if dataset_name == "vggsound":
        file_path = os.path.join(
            root_path, "data/vggsound/VggsoundAVEL40kCategories.txt"
        )
...
class AVE_dataset(Dataset):
    def __init__(self, opt, mode="train"):
        ...
        if self.dataset_name == "vggsound":
            all_df = pd.read_csv(
                os.path.join(opt.root_path, "data/vggsound/vggsound-avel40k_labels.csv")
            )
            self.split_df = all_df[all_df["split"] == mode]

            # 输出train、test、valid的占比
            print(f"{len(self.split_df)}/{len(all_df)} videos are used for {mode}")
            self.all_categories, self.id_to_idx = generate_category_list(
                opt.root_path, self.dataset_name
            )
            ...
            with h5py.File(
                os.path.join(opt.root_path, "data/AVE/labels.h5"), "r"
            ) as hf:
                self.labels = hf["avadataset"][:]

            if mode == "train":
                with h5py.File(
                    os.path.join(opt.root_path, "data/AVE/train_order.h5"), "r"
                ) as hf:
                    order = hf["order"][:]
            elif mode == "test":
                with h5py.File(
                    os.path.join(opt.root_path, "data/AVE/test_order.h5"), "r"
                ) as hf:
                    order = hf["order"][:]

            self.raw_gt = pd.read_csv(
                os.path.join(opt.root_path, "data/AVE/Annotations.txt"), sep="&"
            )
            category_dict = dict()
            for category in self.all_categories:
                category_dict[category] = []
            ...
            if mode == "train":
                self.audio_folder = os.path.join(
                    self.root_path, "data/AVVP/LLP_dataset/audio"
                )
                self.video_folder = os.path.join(
                    self.root_path, "data/AVVP/LLP_dataset/frame"
                )
                self.df = pd.read_csv(
                    os.path.join(self.root_path, "data/AVVP/AVVP_train.csv"),
                    header=0,
                    sep="\t",
                )
                if opt.shot > 0:
                ...
                self.df = pd.read_csv(
                    os.path.join(self.root_path, "data/AVVP/AVVP_test_pd.csv"),
                    header=0,
                    sep="\t",
                )
                self.filenames = self.df["filename"]
    ...
    def getVggoud_proc(self, filename, idx=None):
        ...
        if samples.shape[0] > 16000 * (audio_length + 0.1):
            sample_indx = np.linspace(
                0,
                samples.shape[0] - 16000 * (self.opt.audio_length + 0.1),
                num=10,
                dtype=int,
            )
            samples = samples[
                sample_indx[idx] : sample_indx[idx] + int(16000 * self.opt.audio_length)
            ]

        else:
            # repeat in case audio is too short
            samples = np.tile(samples, int(self.opt.audio_length))[
        ...
        frequencies, times, spectrogram = signal.spectrogram(
            samples, samplerate, nperseg=512, noverlap=353
        )
    ...
    def __getitem__(self, idx):
        if self.dataset_name == "vggsound":
            ...
            category, video_name, labels = (
                one_video_df["category"],
                one_video_df["video_id"],
                one_video_df["label"],
            )
            ...
            total_num_frames = len(
                glob.glob(
                    os.path.join(self.root_path, self.opt.video_folder)
                    + "/"
                    + file_name
                    + "/*.jpg"
                )
            ...
            wave = wave[:, : 32000 * 10]
            # 需要查看一下wave的shape
            return {"GT": self.labels[real_idx], "image": total_img, "wave": wave}
            ...
            total_num_frames = len(
                glob.glob(
                    os.path.join(self.root_path, self.video_folder)
                    + "/"
                    + file_name
                    + "/*.jpg"
                )
            ...
            wave = np.load(
                os.path.join(
                    self.root_path,
                    "data/AVVP/LLP_dataset/wave/{}.npy".format(file_name),
                ),
                allow_pickle=True,
            )
            wave = torch.from_numpy(wave)
            ...
            return {"GT": label, "wave": wave, "image": total_img}
        ...
        return {"GT": label, "image": total_img}
