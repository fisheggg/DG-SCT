...
class AudioVideoInter(nn.Module):
    def __init__(self, d_model, n_head, head_dropout=0.1):
        ...
        self.video_multihead = MultiheadAttention(
            d_model, num_heads=n_head, dropout=head_dropout
        )
...
class CMBS(nn.Module):
    def __init__(self, config):
        ...
        self.video_encoder = InternalTemporalRelationModule(
            input_dim=self.video_input_dim, d_model=self.d_model, feedforward_dim=1024
        )
    ...
    def forward(self, visual_feature, audio_feature):
        ...
        audio_rnn_output1, visual_rnn_output1 = self.audio_visual_rnn_layer(
            audio_rnn_input, visual_rnn_input
        )
        audio_encoder_input1 = audio_rnn_output1.transpose(
            1, 0
        ).contiguous()  # [10, 32, 256]
        visual_encoder_input1 = visual_rnn_output1.transpose(
            1, 0
        ).contiguous()  # [10, 32, 512]

        # audio query
        video_key_value_feature = self.video_encoder(visual_encoder_input1)
        audio_query_output = self.audio_decoder(
            audio_encoder_input1, video_key_value_feature
        )

        # video query
        audio_key_value_feature = self.audio_encoder(audio_encoder_input1)
        video_query_output = self.video_decoder(
            visual_encoder_input1, audio_key_value_feature
        )
        ...
        video_query_output = (
            video_query_output + audio_gate * video_query_output * self.alpha
        )
        audio_query_output = (
            audio_query_output + video_gate * audio_query_output * self.alpha
        )
        ...
        is_event_scores, event_scores = self.localize_module(
            (video_query_output + audio_query_output) / 2
        )
...
class AudioAdapter(nn.Module):
    def __init__(self):
        ...
        self.audio_visual_rnn_layer = RNNEncoder(
            audio_dim=128, video_dim=768, d_model=256, num_layers=1
        )
    ...
    def forward(self, x, audio):
        ...
        audio_key_value_feature = audio_key_value_feature.permute(
            1, 0, 2
        ).contiguous()  # [B, 10, 256]
...
class VisualAdapter(nn.Module):
    ...
    def forward(self, x, vis_token=None, caption=None, is_temporal=False):
        ...
        if self.adapter_kind == "bottleneck" and self.is_multimodal:
            ...
            channel_att_maps_tmp = channel_att_maps.view(
                bs, 10, channel_att_maps.size(-2), channel_att_maps.size(-1)
            )
            spatial_att_maps_sigmoid_tmp = spatial_att_maps_sigmoid.view(
                bs,
                10,
                spatial_att_maps_sigmoid.size(-2),
                spatial_att_maps_sigmoid.size(-1),
            )
            x = x * (
                alpha * channel_att_maps_tmp
                + beta * spatial_att_maps_sigmoid_tmp
                + gamma * temporal_att_maps
                + 1
                - alpha
            )
...
class MMIL_Net(nn.Module):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.classnames, _ = generate_category_list(
            self.opt.root_path, self.opt.dataset_name
        )
        ...
        self.audio_projection = nn.Sequential(
            nn.Linear(768, 512), nn.ReLU(), nn.Linear(512, 512)
        )
        ...
        audio_projection_list = {
            k[24:]: v for k, v in tmp["state_dict"].items() if "audio_projection" in k
        }
        self.audio_projection.load_state_dict(audio_projection_list)

        text_branch_list = {
            k[7:]: v for k, v in tmp["state_dict"].items() if "text_branch" in k
        }
        text_transform_list = {
            k[7:]: v for k, v in tmp["state_dict"].items() if "text_transform" in k
        }
        text_projection_list = {
            k[7:]: v for k, v in tmp["state_dict"].items() if "text_projection" in k
        }
        self.clap_text_encoder = CLAPTextEncoder(
            self.opt,
            self.classnames,
            [text_branch_list, text_transform_list, text_projection_list],
        )
        ...
        useless = [
            "logit_scale_a",
            "logit_scale_t",
            "patch_embed.mel_conv2d.weight",
            "patch_embed.mel_conv2d.bias",
            "patch_embed.fusion_model.local_att.0.weight",
            "patch_embed.fusion_model.local_att.0.bias",
            "patch_embed.fusion_model.local_att.1.weight",
            "patch_embed.fusion_model.local_att.1.bias",
            ...
            "patch_embed.fusion_model.local_att.3.weight",
            "patch_embed.fusion_model.local_att.3.bias",
            "patch_embed.fusion_model.local_att.4.weight",
            "patch_embed.fusion_model.local_att.4.bias",
            ...
            "patch_embed.fusion_model.global_att.1.weight",
            "patch_embed.fusion_model.global_att.1.bias",
            "patch_embed.fusion_model.global_att.2.weight",
            "patch_embed.fusion_model.global_att.2.bias",
            ...
            "patch_embed.fusion_model.global_att.4.weight",
            "patch_embed.fusion_model.global_att.4.bias",
            "patch_embed.fusion_model.global_att.5.weight",
            "patch_embed.fusion_model.global_att.5.bias",
            "patch_embed.fusion_model.global_att.5.running_mean",
            "patch_embed.fusion_model.global_att.5.running_var",
            "patch_embed.fusion_model.global_att.5.num_batches_tracked",
            "logit_scale_a",
            "logit_scale_t",
        ]
    ...
    def forward_vit(self, wave, vis):
        b, t, c, w, h = vis.shape
        output_dict = self.lavish_forward(
            rearrange(vis, "b t c w h -> (b t) c w h"), wave
        )  # [B*10, 512]
        v_cls = output_dict["x"]
    ...
    def lavish_forward(self, vis, wave, mixup_lambda=None, longer_idx=None):
        ...
        y = self.htsat.spectrogram_extractor(
            y
        ...
        for idx_blk, blk in enumerate(self.htsat.layers):
            for idx_layer, layer in enumerate(blk.blocks):
                ...
                # compute vis
                x = x + self.ViT.transformer.resblocks[cnt].attention(
                    self.ViT.transformer.resblocks[cnt].ln_1(x)
                )
                ...
                f_a_res, spatial_att_maps, temporal_att_maps = (
                    self.audio_adapter_blocks_p1[
                        cnt
                    ](
                ...
                f_v_res, spatial_att_maps, temporal_att_maps = (
                    self.vis_adapter_blocks_p1[
                        cnt
                    ](
                        f_v.permute(0, 2, 1).unsqueeze(-1),
                        f_a.permute(0, 2, 1).unsqueeze(-1),
                ...
                x = f_v.permute(1, 0, 2)
                x = x + self.ViT.transformer.resblocks[cnt].mlp(
                    self.ViT.transformer.resblocks[cnt].ln_2(x)
                )

                f_a_res, spatial_att_maps, temporal_att_maps = (
                    self.audio_adapter_blocks_p2[
                        cnt
                    ](
                        f_a.permute(0, 2, 1).unsqueeze(-1),
                        f_v.permute(0, 2, 1).unsqueeze(-1),
        ...
        SF = (
            frames_num
            // (2 ** (len(self.htsat.depths) - 1))
            // self.htsat.patch_stride[0]
        )
        ST = (
            frames_num
            // (2 ** (len(self.htsat.depths) - 1))
            // self.htsat.patch_stride[1]
        )
        ...
        fpx = interpolate(
            torch.sigmoid(y).permute(0, 2, 1).contiguous(),
            8 * self.htsat.patch_stride[1],
        )
        ...
        logits_audio_image, logits_image_audio = self.audio_visual_contrastive_learner(
            x, latent_output
        )
        output_dict = {
            "framewise_output": fpx,  # already sigmoided
