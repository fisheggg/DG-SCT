...
elif args.dataset_name == "LLP":
    args.audio_folder = "data/AVVP/LLP_dataset/audio"
    args.video_folder = "data/AVVP/LLP_dataset/frame"
...
def train(args, model, train_loader, optimizer, criterion, criterion_event, epoch):
    ...
    optimizer.zero_grad()
    mean_loss = torch.zeros(1).to("cuda")
    mean_loss_audio_image = torch.zeros(1).to("cuda")
    mean_loss_image_audio = torch.zeros(1).to("cuda")
    mean_acc = torch.zeros(1).to("cuda")

    for batch_idx, sample in enumerate(train_loader):
        gt = sample["GT"]
        # print("gt", gt.shape)
        image = sample["image"]
        audio_vgg = sample["audio_vgg"].float()
        wave = sample["wave"]

        bs = image.size(0)

        if args.dataset_name in ["AVE", "vggsound", "LLP"]:
            event_scores, logits_audio_image, logits_image_audio = model(
                wave.to("cuda"), image.to("cuda")
            )
            if args.weak and args.dataset_name in ["AVE", "vggsound"]:
                gt = gt[:, :, :-1]
                gt_list = []
                for gt_i in gt:
                    ready = False
        ...
        if args.dataset_name in ["AVE", "vggsound", "LLP"]:
            if args.weak:
                ...
                loss = criterion_event(
                    event_scores, rearrange(labels, "b t class -> (b t) class")
                )
        else:
            raise NotImplementedError

        audio_visual_labels = torch.eye(bs).to("cuda")
        ...
        if epoch <= 10:
            w1 = loss / (epoch + loss + loss_audio_image + loss_image_audio) + 1 / epoch
            w2 = (
                loss_audio_image / (epoch + loss + loss_audio_image + loss_image_audio)
                + 1 / epoch
            )
            w3 = (
                loss_image_audio / (epoch + loss + loss_audio_image + loss_image_audio)
                + 1 / epoch
            )
        ...
        mean_loss_image_audio = (
            mean_loss_image_audio * batch_idx + loss_image_audio.detach()
        ) / (batch_idx + 1)
        mean_loss_audio_image = (
            mean_loss_audio_image * batch_idx + loss_audio_image.detach()
        ) / (batch_idx + 1)
        """Compute Accuracy"""
        if not args.weak:
            assert args.dataset_name != "LLP"
            val = (
                event_scores.argmax(dim=-1)
                == rearrange(labels, "b t class -> (b t) class").argmax(dim=-1)
            ).sum()
        else:
        ...
        if batch_idx % 50 == 0:
            ...
            print(
                "Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss_total: {:.6f}".format(
                    epoch,
                    batch_idx * bs,
            ...
            print(
                "loss_is_event: {:.6f} loss_audio_image: {:.6f} loss_image_audio: {:.6f}".format(
                    (mean_loss - mean_loss_image_audio - mean_loss_audio_image).item(),
                    mean_loss_audio_image.item(),
                    mean_loss_image_audio.item(),
                )
            )
...
def eval(model, val_loader, args):
    ...
    for batch_idx, sample in enumerate(val_loader):
        ...
        labels = gt.to("cuda")
        if args.dataset_name in ["AVE", "vggsound", "LLP"]:
            # event_scores, _, _ = model(audio_vgg.to('cuda'), image.to('cuda'))
            event_scores, _, _ = model(wave.to("cuda"), image.to("cuda"))
            if not args.classification:
                assert (
                    args.dataset_name != "LLP"
                ), "LLP dataset can only do classification task!"
                val = (
                    event_scores.argmax(dim=-1)
                    == rearrange(labels, "b t class -> (b t) class").argmax(dim=-1)
                ).sum()
...
def main():
    ...
    if args.resume != "":
        ...
        tmp = {
            ...
            if k[7:]
            not in ["prompt_learner.token_prefix", "prompt_learner.token_suffix"]
        }
    ...
    # model = torch.nn.parallel.DistributedDataParallel(model, 'cuda'_ids=[args.gpu], find_unused_parameters=True)
    model.to("cuda")
    ...
    if args.mode == "train":
        ...
        print(
            "####### Trainable params: %0.4f  #######"
            % (train_params * 100 / total_params)
        )
        print(
