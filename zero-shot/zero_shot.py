...
args.parser.add_argument(
    "--zero_audio_folder", type=str, default="", help="raw audio path for zero-shot"
)
args.parser.add_argument(
    "--zero_video_folder", type=str, default="", help="video frame path for zero-shot"
)
args.parser.add_argument(
    "--backbone", type=str, default="", help="name of the pretrained model"
)
args.parser.add_argument(
    "--is_event_score", type=int, default=1, help="compute is_event_score"
)
args.parser.add_argument(
    "--test_dataset_name", type=str, default="", help="the downstream dataset"
)
...
pretrain_model.load_state_dict(
    {
        ...
        if "prompt_learner.token_prefix" not in k
        and "prompt_learner.token_suffix" not in k
    },
    strict=False,
)
        ...
        if dataset_name == "AVE":
            if is_event_score:
                acc = (
                    ...
                    == rearrange(gt, "b t class -> (b t) class").argmax(dim=-1)
                ).sum()
            else:
                gt = gt[:, :, :-1]
                gt_list = []
