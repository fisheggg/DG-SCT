...
class EncoderLayer(Module):
    ...
    def __init__(
        self, d_model, nhead, dim_feedforward=1024, dropout=0.1, activation="relu"
    ):
...
class New_Audio_Guided_Attention(nn.Module):
    ...
    def __init__(self, beta):
        ...
        self.latent_dim = 4
        self.video_query = nn.Linear(
            self.video_input_dim, self.video_input_dim // self.latent_dim
        )
    ...
    def forward(self, video, audio):
        ...
        video_query_feature = self.video_query(visual_feature).reshape(
            batch * t_size, h * w, -1
        )  # [B, h*w, C]
        video_key_feature = (
            self.video_key(visual_feature)
            .reshape(batch * t_size, h * w, -1)
            .permute(0, 2, 1)
        )  # [B, C, h*w]
        energy = torch.bmm(video_query_feature, video_key_feature)
        attention = self.softmax(energy)
        video_value_feature = self.video_value(visual_feature).reshape(
            batch * t_size, h * w, -1
        )
        output = torch.matmul(attention, video_value_feature)
        output = self.norm(
            visual_feature.reshape(batch * t_size, h * w, -1) + self.dropout(output)
        )
        ...
        self_spatial_att_maps = self.softmax(
            self.tanh(self.ave_v_att(self_query)).transpose(2, 1)
        )
        self_att_feat = (
            torch.bmm(self_spatial_att_maps, visual_feature)
            .squeeze()
            .reshape(batch, t_size, v_dim)
        )
        ...
        channel_att_maps = (
            self.affine_v_c_att(audio_video_query)
            .sigmoid()
            .reshape(batch, t_size, -1, v_dim)
        )
        ...
        spatial_att_maps = self.softmax(
            self.tanh(self.affine_v_s_att(audio_video_query_2)).transpose(2, 1)
        )
        c_s_att_visual_feat = (
            torch.bmm(spatial_att_maps, c_att_visual_feat)
            .squeeze()
            .reshape(batch, t_size, v_dim)
        )
        ...
        c_s_att_visual_feat = (
            c_s_att_visual_feat
            + self.beta * self_att_feat.sigmoid() * c_s_att_visual_feat
        )
