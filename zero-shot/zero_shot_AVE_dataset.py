...
class AVE_dataset(Dataset):
    def __init__(self, opt, mode="train"):
        ...
        if self.dataset_name == "AVE":
            ...
            with h5py.File(
                os.path.join(self.root_path, "data/AVE/mil_labels.h5"), "r"
            ) as hf:
    ...
    def __getitem__(self, idx):
        if self.dataset_name == "AVE":
            ...
            wave = np.load(
                "/root/autodl-tmp/duanhaoyi/data/AVVP/LLP_dataset/wave/{}.npy".format(
                    file_name
                ),
                allow_pickle=True,
            )
        ...
        total_num_frames = len(
            glob.glob(self.opt.zero_video_folder + "/" + file_name + "/*.jpg")
        )
