    ...
    def __getitem__(self, idx):
        if self.dataset_name == "vggsound":
            ...
            total_num_frames = len(
                glob.glob(self.opt.video_folder + "/" + file_name + "/*.jpg")
            )
            ...
            ### ---> video frame process
            total_num_frames = len(
                glob.glob(self.video_folder + "/" + file_name + "/*.jpg")
            )
            sample_indx = np.linspace(1, total_num_frames, num=10, dtype=int)
            total_img = []
            for vis_idx in range(10):
            ...
            wave = np.load(
                os.path.join(
                    self.root_path, "AVVP/LLP_dataset/wave/{}.npy".format(file_name)
                ),
                allow_pickle=True,
            )
            wave = torch.from_numpy(wave)
